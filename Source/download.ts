...
export async function downloadLanguageServer(name: string) {
	...
	async function tryToInstall() {
		try {
			...
			if (e == 127) {
				...
				failureNotificationRequest.title = nova.localize(
					"NPM Might Not Be Installed"
				);
				failureNotificationRequest.body = nova.localize(
					"Install NPM and try again."
				);
				failureNotificationRequest.actions = [nova.localize("Retry")];
	...
	const pyrightDownloadLocation = nova.path.join(
		tempDirPath,
		"node_modules",
		"pyright"
	);
